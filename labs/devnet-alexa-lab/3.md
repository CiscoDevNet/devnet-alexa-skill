...
In the case of an Alexa Skill the Lambda function is triggered when the Alexa Skill "hears" one of its' defined utterances. You'll get to set up the utterances on the next page.
...
1. Create as S3 Bucket, replace the `!!` in the command below with **Your AWS Username**.

  - type this command, be sure to replace the `!!` with **Your AWS Username**
...
1. Create the Lambda function and connect it to the code in S3
  ...
  - **You need to replace** the `!!` in the **function name** with **Your AWS Username**

  - **You need to replace** the `!!` in the **S3 bucket name** with **Your AWS Username**

  - **You need to replace** the `0.0.0.0` IP address at the very end of the command with **Your UCS Manager IP Address**. You can find your **UCS Manager IP** in the Lab Resources panel
  ...
  - type this command, **Remember to make the changes listed above before hitting enter!**
    ...
    **Don't Panic**
    ...
    - If you hit enter and the command failed, you probably didn't replace the `!!` with your with **Your AWS Username**. The `!!` in a Lambda function name are not allowed, that's probably why the command failed.
  ...
  - `--function-name` - is what the function will be called

  - `--runtime` - is the language that AWS Lambda will use to run your code
  ...
  - `--environment Variables={UCSMHOST=0.0.0.0}` - is the way you indicate the IP address of **Your UCS Manager**
...
3. Select the Your Lambda Function, remember **Your AWS Username** will be in the function name and **not** `!!`

  **Be Sure** to pick **YOUR** Lambda function, this is a shared environment so you may see the Lambda functions that other people have created.
...
Go to Page 4 - **Create the Alexa Skill**
