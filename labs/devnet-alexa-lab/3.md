...
1. Create the Lambda function and connect it to the code in S3.
  ...
  The command to create the Lambda function is long, and it is shown in the box below. **You need to make THREE CHANGES to the command**.

  - **You need to replace** the `!!` in the **function name** with **Your AWS Username** from the lab resources panel.

  - **You need to replace** the `!!` in the **S3 bucket name** with **Your AWS Username** from the lab resources panel.

  - **You need to replace** the `0.0.0.0` IP address at the end of the command with **Your UCS Manager IP Address**. You can find your **UCS Manager IP** in the lab resources panel.
